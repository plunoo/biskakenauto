...
router.get('/dashboard', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    try {
        ...
        const [totalJobs, completedJobs, pendingJobs, inProgressJobs, totalCustomers, totalInvoices, paidInvoices, totalRevenue, monthlyRevenue, lowStockItems, recentJobs, overdueInvoices] = await Promise.all([
            prisma_1.prisma.job.count({ where }),
            prisma_1.prisma.job.count({ where: { ...where, status: 'COMPLETED' } }),
            prisma_1.prisma.job.count({ where: { ...where, status: 'PENDING' } }),
            prisma_1.prisma.job.count({ where: { ...where, status: 'IN_PROGRESS' } }),
            ...
            prisma_1.prisma.inventory.count({
                where: {
                    stockQty: { lte: prisma_1.prisma.inventory.fields.reorderLevel }
                }
            }),
...
router.get('/revenue', (0, auth_1.authorize)('ADMIN', 'SUB_ADMIN'), (0, errorHandler_1.asyncHandler)(async (req, res) => {
    try {
        const { from, to } = req.query;
        const startDate = from ? new Date(from) : new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
        const endDate = to ? new Date(to) : new Date();
        // Revenue by payment method
        const revenueByMethod = await prisma_1.prisma.invoice.groupBy({
            by: ['paymentMethod'],
            where: {
                status: 'PAID',
                paymentDate: {
        ...
        const dailyRevenue = await prisma_1.prisma.invoice.findMany({
            where: {
                status: 'PAID',
                paymentDate: {
                    gte: startDate,
        ...
        // Group by date
        const dailyBreakdown = dailyRevenue.reduce((acc, invoice) => {
...
router.get('/inventory', (0, auth_1.authorize)('ADMIN', 'SUB_ADMIN'), (0, errorHandler_1.asyncHandler)(async (req, res) => {
    try {
        // Inventory overview
        const [totalItems, totalValue, lowStockItems, outOfStockItems, topUsedParts] = await Promise.all([
            prisma_1.prisma.inventory.count(),
            ...
            prisma_1.prisma.jobPart.groupBy({
                by: ['partId'],
                _sum: { quantity: true },
                _count: true,
        ...
        const partDetails = await prisma_1.prisma.inventory.findMany({
            where: { id: { in: partIds } },
...
router.get('/mechanics', (0, auth_1.authorize)('ADMIN', 'SUB_ADMIN'), (0, errorHandler_1.asyncHandler)(async (req, res) => {
    try {
        const { from, to } = req.query;
        const startDate = from ? new Date(from) : new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
        const endDate = to ? new Date(to) : new Date();
        // Get mechanic performance data
        const mechanicStats = await prisma_1.prisma.user.findMany({
            where: {
...
router.get('/ai-insights', (0, auth_1.authorize)('ADMIN', 'SUB_ADMIN'), (0, errorHandler_1.asyncHandler)(async (req, res) => {
    try {
        // Gather business data for AI analysis
        const [totalJobs, completedJobs, totalRevenue, topServices, inventoryAlerts, avgJobTime, customerRetention, monthlyTrend] = await Promise.all([
            ...
            prisma_1.prisma.inventory.count({
                where: { stockQty: { lte: prisma_1.prisma.inventory.fields.reorderLevel } }
            }),
        ...
        // Get AI insights
        const insights = await aiService_1.aiService.generateInsights(businessData);
        res.json({
            success: true,
            data: {
                businessData,
...
router.post('/export', (0, auth_1.authorize)('ADMIN', 'SUB_ADMIN'), (0, errorHandler_1.asyncHandler)(async (req, res) => {
    try {
        ...
        // Generate PDF
        const pdfPath = await pdfService_1.pdfService.generateJobReport(reportData);
        // Send file
