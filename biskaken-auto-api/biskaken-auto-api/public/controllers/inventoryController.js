...
exports.inventoryController = {
    ...
    async list(req, res) {
        try {
            ...
            const total = await prisma_1.prisma.inventory.count({ where });
    ...
    async create(req, res) {
        try {
            ...
            const existingPart = await prisma_1.prisma.inventory.findFirst({
                where: {
            ...
            const item = await prisma_1.prisma.inventory.create({
                data: {
                    partName,
                    category,
                    stockQty,
    ...
    async update(req, res) {
        try {
            ...
            const existingItem = await prisma_1.prisma.inventory.findUnique({
                where: { id }
            });
            if (!existingItem) {
                return res.status(404).json({
            ...
            if (updateData.partName && updateData.partName !== existingItem.partName) {
                const nameConflict = await prisma_1.prisma.inventory.findFirst({
                    where: {
            ...
            const updatedItem = await prisma_1.prisma.inventory.update({
                where: { id },
                data: updateData
            });
            // Log activity
    ...
    async getLowStock(req, res) {
        try {
            const lowStockItems = await prisma_1.prisma.inventory.findMany({
                where: {
                    OR: [
    ...
    async getAIPrediction(req, res) {
        try {
            ...
            // Get AI prediction
            const prediction = await aiService_1.aiService.predictReorder({
                partName: item.partName,
    ...
    async getUsageReport(req, res) {
        try {
            ...
            const usageData = await prisma_1.prisma.jobPart.findMany({
                where: {
                    createdAt: {
                        gte: startDate,
                        lte: endDate
            ...
            const reportData = Object.values(usageByPart)
                .sort((a, b) => b.totalUsed - a.totalUsed) // Sort by most used
