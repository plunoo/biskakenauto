...
exports.customerController = {
    ...
    async list(req, res) {
        try {
            ...
            const total = await prisma_1.prisma.customer.count({ where });
            res.json({
                success: true,
                data: customers,
                pagination: {
    ...
    async create(req, res) {
        try {
            ...
            const customerWithVehicle = await prisma_1.prisma.customer.findUnique({
    ...
    async update(req, res) {
        try {
            ...
            if (phone && phone !== existingCustomer.phone) {
                const phoneConflict = await prisma_1.prisma.customer.findUnique({
                    where: { phone }
                });
                if (phoneConflict) {
                    return res.status(409).json({
    ...
    async getHistory(req, res) {
        try {
            ...
            const total = await prisma_1.prisma.job.count({
                where: { customerId: id }
            });
            res.json({
                success: true,
