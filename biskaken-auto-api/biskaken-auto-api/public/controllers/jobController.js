...
exports.jobController = {
    ...
    async create(req, res) {
        try {
            ...
            // Generate job number
            const jobCount = await prisma_1.prisma.job.count();
    ...
    async assign(req, res) {
        try {
            ...
            const mechanic = await prisma_1.prisma.user.findUnique({
                where: {
                    id: mechanicId,
                    status: 'ACTIVE'
                }
    ...
    async getStats(req, res) {
        try {
            ...
            const [total, pending, inProgress, completed, cancelled, highPriority, overdue] = await Promise.all([
                ...
                prisma_1.prisma.job.count({ where: { ...where, status: 'CANCELLED' } }),
                prisma_1.prisma.job.count({ where: { ...where, priority: 'HIGH' } }),
