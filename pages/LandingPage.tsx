...
const BlogCard = ({ image, category, title, date, readTime, slug, id }: any) => (
...
const LandingPage: React.FC = () => {
  ...
  const loadLandingBlogData = async () => {
    try {
      ...
      const blogRes = await apiService.getLandingBlog();
      
      if (blogRes.success) {
        setBlogPosts(blogRes.data);
        console.log('✅ Landing blog data loaded:', blogRes.data.length, 'posts');
      }
    } catch (error) {
