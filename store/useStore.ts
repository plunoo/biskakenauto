...
export const useStore = create<AppState>((set, get) => ({
  ...
  addJob: (job) => {
    ...
    // Send SMS notification to customer when new job is created
    if (customer && customer.phone) {
      const vehicleInfo = `${customer.vehicle.make} ${customer.vehicle.model} (${customer.vehicle.plateNumber})`;
      const message = `Hello ${customer.name}, we've received your vehicle (${vehicleInfo}) for repair. Job ID: ${job.id}. We'll keep you updated on the progress. - Biskaken Auto`;
      ...
      smsService.sendSMS({
        ...
        if (result.success) {
          console.log('✅ Job creation SMS sent successfully:', result.messageId);
        } else {
          console.error('❌ Failed to send job creation SMS:', result.error);
        }
      }).catch(error => {
        console.error('❌ SMS error:', error);
        // SMS error logged to console
      });
    }
  },
  ...
  updateJobStatus: (jobId, status) => {
    ...
    // Send SMS notification to customer when status changes
    if (job && customer && customer.phone) {
      const vehicleInfo = `${customer.vehicle.make} ${customer.vehicle.model} (${customer.vehicle.plateNumber})`;
      smsService.sendJobStatusUpdate(
        customer.phone,
        customer.name,
        jobId,
        status,
  ...
  recordPayment: (invoiceId, payment) => {
    ...
    if (customer && customer.phone) {
      smsService.sendPaymentConfirmation(
        ...
        if (result.success) {
          console.log('✅ Payment SMS sent successfully:', result.messageId);
        } else {
          console.error('❌ Payment SMS failed:', result.error);
        }
      });
    }
