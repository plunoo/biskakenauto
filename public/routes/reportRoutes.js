...
router.get('/revenue', authorize('ADMIN', 'SUB_ADMIN'), asyncHandler(async (req, res) => {
    try {
...
router.get('/inventory', authorize('ADMIN', 'SUB_ADMIN'), asyncHandler(async (req, res) => {
    try {
...
router.get('/mechanics', authorize('ADMIN', 'SUB_ADMIN'), asyncHandler(async (req, res) => {
    try {
        const { from, to } = req.query;
        const startDate = from ? new Date(from) : new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
        const endDate = to ? new Date(to) : new Date();
        ...
        const mechanicPerformance = mechanicStats.map(mechanic => {
            ...
            const avgCompletionTime = completedJobsWithTime.length > 0
                ...
                }, 0) / completedJobsWithTime.length / (1000 * 60 * 60 * 24)
                : 0;
            return {
...
router.get('/ai-insights', authorize('ADMIN', 'SUB_ADMIN'), asyncHandler(async (req, res) => {
    try {
...
router.post('/export', authorize('ADMIN', 'SUB_ADMIN'), asyncHandler(async (req, res) => {
    try {
